...
test.describe('System Test: Delete Pay Grade Workflow', () => {
  ...
  const newPayGradeName = `GradeDeleteTest-${Date.now()}`; // ✅ Template literal corrigido

  test('Full Pay Grade workflow: Create, Delete, Verify', async ({ page }) => {
    ...
    await test.step('Navegação para Admin > Job > Pay Grades', async () => {
    ...
    await test.step('Criação de um novo Pay Grade', async () => {
    ...
    await test.step('Exclusão do Pay Grade criado', async () => {
    ...
    await test.step('Verificação de exclusão', async () => {
      await page.goto('https://opensource-demo.orangehrmlive.com/web/index.php/admin/viewPayGrades');
